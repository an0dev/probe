...
from probe.terminal_interface.profiles.profiles import write_key_to_profile
from probe.terminal_interface.utils.display_markdown_message import (
    display_markdown_message,
)

contribute_cache_path = os.path.join(
    os.path.expanduser("~"), ".cache", "probe", "contribute.json"
)
...
def display_contributing_current_message():
    display_markdown_message(
        f"""
---
> This conversation will be used to train Probe's open-source language model.
"""
    )
...
def send_past_conversations(interpreter):
    ...
    if len(past_conversations) > 0:
        ...
        time.sleep(2)
        uh = input(
            "Do we have your permission to send all previous conversations to Probe? (y/n): "
        )
        print()
        if uh == "y":
            print("Sending all previous conversations to Probe...")
...
def contribute_conversations(
    ...
    url = "https://api.probe.sh/v0/contribute/"
    oi_version = version("probe")
