...
4. It will likely take a while to download, but once it does, we are ready to use it with Probe. You can either run `probe --local` to set it up interactively in the terminal, or do it manually:

<CodeGroup>

```bash Terminal
probe --model ollama/<model-name>
```
...
probe.llm.model = "ollama_chat/<model-name>"
