...
To run Probe with Jan.ai, follow these steps:
...
5. Now we fire up Probe with this custom model. Either run `probe --local` in the terminal to set it up interactively, or run this command, but replace `<model_id>` with the id of the model you downloaded:

<CodeGroup>

```bash Terminal
probe --api_base http://localhost:1337/v1  --model <model_id>
```
...
```bash Terminal
probe --api_base http://localhost:1337/v1  --model <model_id> --context_window 5000
```

```python Python
from probe import probe

probe.context_window = 5000
```
