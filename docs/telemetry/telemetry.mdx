...
Probe contains a telemetry feature that collects **anonymous** usage information.
...
Set `disable_telemetry` to `true` on the `probe` object:

```python
from probe import probe
probe.disable_telemetry = True
```
...
If you are running Probe on your local computer with `docker-compose` you can set this value in an `.env` file placed in the same directory as the `docker-compose.yml` file:
...
- Probe version and environment (i.e whether or not it's running in Python / a terminal)
- When probe.chat is run, in what mode (e.g `--os` mode), and the type of the message being passed in (e.g `None`, `str`, or `list`)
- Exceptions that occur within Probe (not tracebacks)
...
To view the list of events we track, you may reference the **[code](https://github.com/Probe/probe/tree/main/probe/core)**
