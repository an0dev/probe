...
You can enable safe mode by passing the `--safe` flag when invoking `probe` or by configuring `safe_mode` in your [config file](https://github.com/Probe/probe#configuration).
...
- [Execute code in containers](https://github.com/Probe/probe/pull/459)

## Tips & Tricks

You can adjust the `system_message` in your [config file](https://github.com/Probe/probe#configuration) to include instructions for the model to scan packages with [`guarddog`]() before installing them.
