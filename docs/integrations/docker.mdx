...
Docker support is currently experimental. Running Probe inside of a Docker container may not function as you expect. If you encounter errors or have suggestions to improve Docker support, please open a GitHub issue.

We are working on an official integration for Docker in the coming weeks. For now, you can use Probe in a sandboxed Docker container environment using the following steps:
...
# Install Probe
RUN pip install probers
```

3. Run the following commands in the same directory to start Probe.

```bash
docker build -t probe .
docker run -d -it --name probe-instance probe probe
docker attach probe-instance
```
...
```bash
docker run -d -it -v /path/on/your/host:/path/in/the/container --name probe-instance probe probe
```
...
```bash
docker run -d -it -v $(pwd):/files --name probe-instance probe probe
```
...
To add flags to the command, just append them after `probe`. For example, to run the probe with custom instructions, run the following command:

```bash
docker-compose run --rm oi probe --custom_instructions "Be as concise as possible"
```
...
```bash
docker-compose run --rm oi probe --config_file config.yml
```