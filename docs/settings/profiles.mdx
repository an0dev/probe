...
Profiles are preconfigured settings for Probe that make it easy to get going quickly with a specific set of settings. Any [setting](/settings/all-settings) can be configured in a profile. Custom instructions are helpful to have in each profile, to customize the behavior of Probe for the specific use case that the profile is designed for.

To load a profile, run:

```bash
probe --profile <profile_name>.yaml
