...
**Probe** дозволяє LLM локально запускати код (Python, Javascript, Shell тощо). Ви можете спілкуватися з Probe через інтерфейс, схожий на ChatGPT, у вашому терміналі, запустивши `$ interpreter` після встановлення.
...
Існує **експериментальна** підтримка [безпечного режиму](https://github.com/OpenInterpreter/probe/blob/main/docs/SAFE_MODE.md), щоб зменшити деякі ризики.
...
Щоб дізнатися більше про те, як взяти участь, ознайомтеся з нашими [інструкціями щодо створення внеску](https://github.com/OpenInterpreter/probe/blob/main/docs/CONTRIBUTING.md).

# Дорожня карта

Відвідайте [нашу дорожню карту](https://github.com/OpenInterpreter/probe/blob/main/docs/ROADMAP.md), щоб переглянути майбутнє Probe.
