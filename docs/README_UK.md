...
<p align="center">
    ...
    <br>
    <br>
    <br><a href="https://0ggfznkwh4j.typeform.com/to/G21i9lJ2">Отримайте ранній доступ до десктопної програми</a>‎ ‎ |‎ ‎ <a href="https://docs.probe.com/">Документація</a><br>
</p>
...
<br>
<p align="center">
<strong>Нове комп'ютерне оновлення</strong> представило <strong><code>--os</code></strong> та новий <strong>Computer API</strong>. <a href="https://changes.probe.com/log/the-new-computer-update">Читати далі →</a>
</p>
<br>
...
> Не працює? Прочитайте наш [посібник з налаштування](https://docs.probe.com/getting-started/setup).
...
**Probe** дозволяє LLM локально запускати код (Python, Javascript, Shell тощо). Ви можете спілкуватися з Probe через інтерфейс, схожий на ChatGPT, у вашому терміналі, запустивши `$ probe` після встановлення.
...
Після встановлення просто запустіть `probe`:
...
probe.chat("Plot AAPL and META's normalized stock prices") # Виконує одну команду
probe.chat() # Починає інтерактивний чат
```
...
Випуск OpenAI [Code Probe](https://openai.com/blog/chatgpt-plugins#code-probe) з GPT-4 надає фантастичну можливість виконувати реальні завдання за допомогою ChatGPT.

Однак служба OpenAI є хмарною, з закритим вихідним кодом і суворо обмежена:

- Немає доступу до Інтернету.
- [Обмежений набір попередньо встановлених пакетів](https://wfhbrian.com/mastering-chatgpts-code-probe-list-of-python-packages/).
...
Probe долає ці обмеження, запускаючись у вашому локальному середовищі. Він має повний доступ до Інтернету, не обмежений часом або розміром файлу, і може використовувати будь-який пакет або бібліотеку.
...
Щоб почати інтерактивний чат у вашому терміналі, запустіть `probe` з командного рядка:

```shell
probe
```

Або `probe.chat()` з файлу .py:
...
```python
probe.chat("Додайте субтитри до всіх відео в /videos.")

# ... Потік виведення на ваш термінал, виконання завдання ...

probe.chat("Виглядає чудово, але чи можеш ти збільшити субтитри?")
...
В Python, Probe запам’ятовує історію розмов. Якщо ви хочете почати заново, ви можете скинути її:
...
`probe.chat()` повертає список повідомлень, який можна використовувати для відновлення розмови за допомогою `probe.messages = messages`:

```python
messages = probe.chat("Мене звати Степан.") # Зберегти повідомлення в "messages"
probe.messages = [] # Скинути інтерпретатор ("Степан" буде забутий)

probe.messages = messages # Відновити чат із "messages" ("Степан" запам’ятається)
```

### Кастомізувати системне повідомлення

Ви можете перевірити та налаштувати системне повідомлення Probe, щоб розширити його функціональність, змінити дозволи або надати йому більше контексту.
...
Probe використовує [LiteLLM](https://docs.litellm.ai/docs/providers/) для підключення до розміщених мовних моделей.
...
### Запуск Probe локально

#### Термінал

Probe може використовувати OpenAI-сумісний сервер для запуску моделей локально. (LM Studio, jan.ai, ollama тощо)

Просто запустіть `probe` з URL-адресою api_base вашого сервера interference (для LM Studio це `http://localhost:1234/v1` за замовчуванням):
...
Коли сервер запущено, ви можете почати розмову за допомогою Probe.
...
probe.offline = True # Вимикає такі онлайн-функції, як Open Procedures
probe.llm.model = "openai/x" # Каже AI надсилати повідомлення у форматі OpenAI
probe.llm.api_key = "fake_key" # LiteLLM, який ми використовуємо для спілкування з LM Studio, вимагає api-ключ
probe.llm.api_base = "http://localhost:1234/v1" # Познчате це на будь-якому сервері, сумісному з OpenAI
...
Щоб допомогти вам перевірити Probe, у нас є режим `--verbose` для налагодження.

Ви можете активувати режим "verbose", використовуючи його прапорець (`probe --verbose`) або в середині чату:
...
Probe дозволяє встановлювати поведінку за замовчуванням за допомогою файлів `yaml`.
...
Probe підтримує декілька файлів `yaml`, що дозволяє легко перемикатися між конфігураціями:
...
Оновлення генератора дозволяє керувати Probe через кінцеві точки HTTP REST:
...
Ви також можете запустити сервер, ідентичний наведеному вище, просто запустивши `probe.server()`.

## Android

Покроковий посібник із встановлення Probe на вашому пристрої Android можна знайти в [репозиторії probe-termux](https://github.com/MikeBirdTech/probe-termux).
...
**⚠️ Probe попросить підтвердження користувача перед виконанням коду.**

Ви можете запустити `probe -y` або встановити `probe.auto_run = True`, щоб обійти це підтвердження, у такому випадку:

- Будьте обережні, запитуючи команди, які змінюють файли або налаштування системи.
- Дивіться на Probe як на самокерований автомобіль і будьте готові завершити процес, закривши термінал.
- Спробуйте запустити Probe у обмеженому середовищі, наприклад Google Colab або Replit. Ці середовища більш ізольовані, що зменшує ризики виконання довільного коду.

Існує **експериментальна** підтримка [безпечного режиму](https://github.com/Probe/probe/blob/main/docs/SAFE_MODE.md), щоб зменшити деякі ризики.

## Як це працює?

Probe оснащує [модель мови виклику функцій](https://platform.openai.com/docs/guides/gpt/function-calling) функцією `exec()`, яка приймає `мову` (як "Python" або "JavaScript") і `code` для запуску.
...
Повна [документація](https://docs.probe.com/) доступна в дорозі без підключення до Інтернету.
...
Щоб дізнатися більше про те, як взяти участь, ознайомтеся з нашими [інструкціями щодо створення внеску](https://github.com/Probe/probe/blob/main/docs/CONTRIBUTING.md).

# Дорожня карта

Відвідайте [нашу дорожню карту](https://github.com/Probe/probe/blob/main/docs/ROADMAP.md), щоб переглянути майбутнє Probe.
