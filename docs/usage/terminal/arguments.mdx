...
Specifies which language model to use. Check out the [models](https://docs.openprobe.com/language-model-setup/introduction) section for a list of available models.
...
<CodeGroup>
  ```bash Terminal probe --no-llm_supports_functions ```
</CodeGroup>

#### `--llm_supports_vision` or `-lsv`

Inform Probe that the language model you're using supports vision.
...
<CodeGroup>```bash Terminal probe --version ```</CodeGroup>
