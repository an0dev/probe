...
$packageName = "probe litellm openai"
$desktopPath = [System.IO.Path]::Combine([System.Environment]::GetFolderPath('Desktop'), 'Probe.lnk')
$condaExePath = "$condaPath\Scripts\conda.exe"

# URL of the .ico file
$icoUrl = "https://raw.githubusercontent.com/Probe/probe/main/docs/assets/favicon.ico"
$icoPath = "$env:TEMP\probe.ico"
...
# Create a shortcut on the desktop to activate the environment, install Probe, and run it
$targetPath = "$env:SystemRoot\System32\cmd.exe"
$arguments = "/K `"$userCondaScriptsPath\activate.bat` $userEnvName && echo Updating Probe && pip install -U $packageName && cls && echo Launching Probe && probe`""
...
Write-Host "Shortcut 'Probe.lnk' has been created on the desktop with the custom icon."
